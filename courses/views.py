...
def profile_view(request):
    ...
    with connection.cursor() as cursor:
        ...
        # ✅ Correctly calculate the user's rank
        cursor.execute("""
            SELECT user_id, RANK() OVER (ORDER BY xp_points DESC) AS rank
            FROM courses_userxp
        """)
...
QUESTS = [
    ...
    {"id": 2, "title": "Earn 500 XP", "goal": 1000, "xp_reward": 100},
...
def progress_dashboard_view(request):
    ...
    with connection.cursor() as cursor:
        # Fetch user XP, streak, and level
        cursor.execute("""
            SELECT xp_points, user_level FROM courses_user WHERE user_id = %s
            """, [user_id])
        user_data = cursor.fetchone()
        xp_points, user_level = user_data

        # Fetch completed lessons
        cursor.execute("""
            SELECT lessons.lesson_id, lessons.lesson_title, progress.score, progress.completed_at
                FROM courses_userprogress progress 
                JOIN courses_lesson lessons on  progress.lesson_id = lessons.lesson_id
        ...
        cursor.execute("""
            SELECT courses_achievement.title, courses_achievement.description, courses_userachievement.achieved_at
                FROM courses_userachievement
                JOIN courses_achievement ON courses_userachievement.achievement_id = courses_achievement.achievement_id
                WHERE courses_userachievement.user_id = %s
                ORDER BY courses_userachievement.achieved_at DESC
            """, [user_id])
